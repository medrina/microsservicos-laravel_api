...
class ClienteController extends Controller {
     ...
     *       required=true,
     *       description=
     *          "<p><b>Pré-condição:</b></p><ul><li><p>dados de cliente preenchidos nos atributos do json (conforme exemplo abaixo).</p></li><p><b>NOTA 1: </b>a API aplicará as regras de validação nos atributos do json da requisição.</p><p><b>NOTA 2 <sup>cpf</sup>: </b>o CPF do cliente precisa seguir o formato de <b>11 dígitos numéricos.</b></p><p><b>NOTA 3 <sup>cpf</sup>: </b>não é possível cadastrar um CPF do cliente já existente na base de dados.</p><p><b>NOTA 4 <sup>data_nasc</sup>: </b>a data de nascimento do cliente deve seguir o formato: <b>YYYY-MM-DD.</b></p><p><b>NOTA 5 <sup>email</sup>: </b>para cadastrar um email do cliente, ele precisa seguir o formato de email: <b>user@email.com</b></p><p><b>NOTA 6 <sup>email</sup>: </b>não é possível cadastrar um email do cliente já existente na base de dados!</p></ul>"
     *       ,
     *       @OA\JsonContent(
     ...
     *    description=
     *       "<p><b>Pré-condição:</b></p><ul><li><p><b>token: </b>usuário administrador deve estar autenticado, e possuir um token válido (obtido no endpoint <b>POST - Efetuar login de usuário Administrador...</b> no microsserviço MSAVALIADOR).</p></li><p><b>NOTA 1 <sup>token</sup>: </b>o token deverá ser preenchido no ícone do cadeado desse endpoint. Após preencher o token no campo <i>Value</i>, clique no botão <i>Authorize</i> e esse endpoint estará autorizado para a plena execução.</p><li><p><b>id: </b>informar id do cliente.</p></li><p><b>NOTA 2 <sup>id</sup>: </b>o id do cliente é de preenchimento obrigatório!</p><p><b>NOTA 3 <sup>id</sup>:</b> para preencher o id, o cliente precisa estar previamente cadastrado no microsserviço MSCLIENTES.</p></ul>"
     *    ,
     *    security={{"bearerAuth": {}}},
     *    @OA\Parameter(
     *       name="id",
     ...
     *       required=true,
     *       description=
     *          "<p><b>Pré-condição:</b></p><ul><li><p>preenchimento/atualização opcional do atributo email no json da requisição (email).</p></li><p><b>NOTA 1 <sup>email</sup>: </b>o preenchimento do atributo email não é obrigatório. Mas, caso o email seja preenchido para ser alterado/atualizado, a API irá aplicar as regras de validação no email.</p><p><b>NOTA 2 <sup>email</sup>: </b>para atualizar o email, ele deve obedecer o seguinte formato de email: <b>user@email.com</b></p></ul>"
     *       ,
     *       @OA\JsonContent(
     ...
     *    description=
     *       "<p><b>Pré-condição:</b></p><ul><li><p><b>token: </b>usuário administrador deve estar autenticado, e possuir um token válido (obtido no endpoint <b>POST - Efetuar login de usuário Administrador...</b> no microsserviço MSAVALIADOR).</p></li><p><b>NOTA <sup>token</sup>: </b>o token deverá ser preenchido no ícone do cadeado desse endpoint. Após preencher o token no campo <i>Value</i>, clique no botão <i>Authorize</i> e esse endpoint estará autorizado para a plena execução.</p><li><p><b>id: </b>informar id do cliente.</p></li><p><b>NOTA 1 <sup>id</sup>:</b> para preencher o id, o cliente precisa estar previamente cadastrado no microsserviço MSCLIENTES.</p><p><b>NOTA 2 <sup>id</sup>: </b>se a busca do cliente for encontrado pelo seu id, o registro <u>NÃO</u> será removido permanentemente, contudo receberá o status de <i>disabled</i> (desativado).</p><p><b>NOTA 3: <sup>id</sup></b>conforme citado na NOTA 2, os cartões vinculados ao cliente também não serão removidos permanentemente da base de dados.</p><p><b>INFORMAÇÃO COMPLEMENTAR: </b>a condição de não excluir o registro permanentemente, existe para manter um histórico de cartões vinculados ao cliente, caso ele seja apagado. Essa condição também permite que o cliente apagado seja reativado novamente.</p></ul>"
     *    ,
     *    security={{"bearerAuth": {}}},
     *    @OA\Parameter(
     *       name="id",
     ...
     *    description=
     *       "<p><b>Pré-condição:</b></p><ul><li><p><b>token: </b>usuário administrador deve estar autenticado, e possuir um token válido (obtido no endpoint <b>POST - Efetuar login de usuário Administrador...</b> no microsserviço MSAVALIADOR).</p></li><p><b>NOTA <sup>token</sup>: </b>o token deverá ser preenchido no ícone do cadeado desse endpoint. Após preencher o token no campo <i>Value</i>, clique no botão <i>Authorize</i> e esse endpoint estará autorizado para a plena execução.</p><li><p><b>cpf: </b>informar cpf do cliente.</p></li><p><b>NOTA 1 <sup>cpf</sup>:</b> para preencher o cpf, o cliente precisa estar previamente cadastrado no microsserviço MSCLIENTES.</p><p><b>NOTA 2 <sup>cpf</sup>: </b>digite apenas números no parâmetro do cpf (sem pontos, traços, hífen, letras ou caracteres especiais).</p></ul>"
     *    ,
     *    security={{"bearerAuth": {}}}, 
     *    @OA\Parameter(
     *       name="cpf",
     ...
     *       response="200",
     *       description=
     *          "Busca de cliente encontrado com sucesso!<p><b>Pós-condição</b></p><p>Retorna um json de resposta exibindo dados desse cliente.</p><p><b>NOTA 1: </b>Junto com essa resposta, será exibida a situação cadastral desse cliente no atributo status: <i>enabled_client</i> para cliente ativado | <i>disabled_client</i> para cliente desativado.</p><p><b>NOTA 2: </b>Também será exibida a data em que esse cliente foi cadastrado no atributo <i>data_inicio</i> ou a data em que o cliente foi removido no atributo <i>data_fim .</i></p><p><b>NOTA 3: </b>Se o cliente nunca foi removido, ou foi reativado o seu registro, o atributo data_fim exibirá o valor <i>active .</i></p><p><b>INFORMAÇÃO COMPLEMENTAR: </b>Esse endpoint possui objetivo de verificar o status do cliente (ativo ou desativado). Dentro do json de resposta, será exibido o id desse cliente. Caso o objetivo seja reativar algum cliente que foi apagado, precisa informar o id retornado do cliente no endpoint: <b>POST - Reativar cadastro de cliente (cliente ativo)</b> no microsserviço MSCLIENTES.</p>"
     *       ,
     *       @OA\JsonContent(
     *          type="array",
     *          @OA\Items(
     ...
     *       required=true,
     *       description=
     *          "<p><b>Pré-condição:</b></p><ul><li><p><b>id: </b>informar id do cliente no atributo id no corpo da requisição (dentro do json).</p></li><p><b>NOTA 1 <sup>id</sup>:</b> para preencher o id, o cliente precisa estar previamente cadastrado no microsserviço MSCLIENTES.</p><p><b>NOTA 2 <sup>id</sup>: </b>o id desse cliente que se deseja reativar o seu cadastro, poderá ser obtido pelo endpoint acima: <b>GET - Pesquisar por situação cadastral de cliente pelo seu CPF</b> no microsserviço MSCLIENTES.</p></ul>"
     *       ,
     *       @OA\JsonContent(
     *          required={"id"},
     ...
     *       response="200",
     *       description=
     *          "Cliente reativado com sucesso!<p><b>Pós-condição</b></p><p>Retorna um json de resposta exibindo dados desse cliente.</p><p><b>NOTA 1: </b>Caso o cliente esteja desativado, será exibido no atributo <i>status</i>: <b>reactivated_client</b> e a mensagem no atributo <i>message</i>: <b>Cliente reativado .</b></p><p><b>NOTA 2: </b>Caso o cliente esteja ativado, será exibido no atributo <i>status</i>: <b>active_client</b> e a mensagem no atributo <i>message:</i><b>Cliente encontra-se ativo .</b></p><p><b>NOTA 3: </b>A cada remoção e reativação do cliente, o atributo <i>data_inicio</i> será atualizado sempre pelo último registro de reativação do cliente, ou seja, quando esse endpoint for executado.</p><p><b>INFORMAÇÃO COMPLEMENTAR: </b>Esse endpoint possui um único objetivo: reativar o cliente que foi removido pelo seu id. Uma vez que o cliente volta a estar ativo, ele voltará a ser exibido nos endpoints de pesquisas de clientes (microsserviço MSCLIENTES) e nas avaliações de crédito (microsserviço MSAVALIADOR).</p>"
     *       ,
     *       @OA\JsonContent(
     *          type="array",
     *          @OA\Items(
     ...
     *    description=
     *       "<p><b>Pré-condição:</b></p><ul><li><p><b>token: </b>usuário administrador deve estar autenticado, e possuir um token válido (obtido no endpoint <b>POST - Efetuar login de usuário Administrador...</b> no microsserviço MSAVALIADOR).</p></li><p><b>NOTA <sup>token</sup>: </b>o token deverá ser preenchido no ícone do cadeado desse endpoint. Após preencher o token no campo <i>Value</i>, clique no botão <i>Authorize</i> e esse endpoint estará autorizado para a plena execução.</p><li><p><b>cpf: </b>informar cpf do cliente.</p></li><p><b>NOTA 1 <sup>cpf</sup>:</b> para preencher o cpf, o cliente precisa estar previamente cadastrado no microsserviço MSCLIENTES.</p></li><p><b>NOTA 2 <sup>cpf</sup>: </b>digite apenas números no parâmetro do cpf (sem pontos, traços, hífen, letras ou caracteres especiais).</p><p><b>INFORMAÇÃO COMPLEMENTAR: </b>esse endpoint possui o objetivo de retornar os dados do cliente, quando o id do cliente é desconhecido. Também serve para utilizar nos endpoints que exigem que o id do cliente seja o único dado a ser enviado por parâmetro.</p></ul>"
     *    ,
     *    security={{"bearerAuth": {}}}, 
     *    @OA\Parameter(
     *       name="cpf",
